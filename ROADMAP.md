...
| 19.6 | E2E Hacienda sandbox | Test suite contra sandbox real de Hacienda (Staging). **Prerequisitos documentados en `docs/hacienda/FACTURACION_ELECTRONICA_SETUP.md`:** (1) Generar credenciales de pruebas en Oficina Virtual (OVi) > Tico Factura (usuario formato `cpf-XX-XXXX-XXXX@stag.comprobanteselectronicos.go.cr` + contraseña ~20 chars). (2) Generar llave criptográfica de pruebas (.p12 + PIN 4 dígitos) desde OVi > "Credenciales de pruebas". **OAuth 2.0 corregido:** `grant_type=password` (no `client_credentials`), `client_id=api-stag`, envía `username`/`password` al IdP. Token URL: `idp.comprobanteselectronicos.go.cr/auth/realms/rut-stag/...`. `access_token` expira en 5 min, `refresh_token` en 10h. Logout obligatorio al LOGOUT_URL enviando `refresh_token`. **Implementado:** `OAuthTokenManager` actualizado con flujo password, refresh_token real, método `logout()`. Config `hacienda.php` y `.env.example` actualizados con `username`, `password`, logout URLs por ambiente. Tests unitarios adaptados (55 tests passing). **Pendiente:** Test suite E2E que ejecute flujo completo (auth → generar XML v4.4 → firmar XAdES-EPES con llave de pruebas → enviar a sandbox → consultar estado → logout) con credenciales reales de OVi. |

...
class PagoCuentaCobrarServiceTest extends TestCase
{
    protected PagoCuentaCobrarService $service;
    private \App\Models\Empresa $empresa;

    protected function setUp(): void
    {
        parent::setUp();
        $this->service = new PagoCuentaCobrarService();
    ...
    private function crearPago(CuentaPorCobrar $cuenta, array $override = []): PagoCuentaCobrar
    {
    ...
    {
        $cuenta = $this->crearCuentaPorCobrar();
    ...
    #[Test]
    public function listar_filtra_por_cuenta_por_cobrar_id(): void
    {
        $cuenta1 = $this->crearCuentaPorCobrar();
        $cuenta2 = $this->crearCuentaPorCobrar();
        $this->crearPago($cuenta1);
        $this->crearPago($cuenta2);

        $resultado = $this->service->listar(['cuenta_por_cobrar_id' => $cuenta1->id]);

        foreach ($resultado->items() as $pago) {
            $this->assertEquals($cuenta1->id, $pago->cuenta_por_cobrar_id);
        }
    }
    ...
    {
        $cuenta = $this->crearCuentaPorCobrar();
        $this->crearPago($cuenta, ['fecha_pago' => '2025-02-15']);
        $this->crearPago($cuenta, ['fecha_pago' => '2025-07-15']);
        ...
        foreach ($resultado->items() as $pago) {
            $this->assertGreaterThanOrEqual('2025-02-01', $pago->fecha_pago->format('Y-m-d'));
            $this->assertLessThanOrEqual('2025-02-28', $pago->fecha_pago->format('Y-m-d'));
        }
    }
    ...
    public function crear_pago_exitosamente(): void
    {
        ...
        $this->assertInstanceOf(PagoCuentaCobrar::class, $pago);
    ...
    {
        $cuenta = $this->crearCuentaPorCobrar(['monto_original' => 10000, 'monto_pagado' => 8000]);
        $formaPago = $this->getFormaPago();
    ...
    public function crear_carga_relaciones(): void
    {
        ...
        $this->assertTrue($pago->relationLoaded('cuentaPorCobrar'));
    ...
    public function eliminar_pago_revierte_monto_pagado(): void
    {
        ...
        $this->assertDatabaseHas('pagos_cuentas_cobrar', [
            'id' => $pago->id,
            'activo' => false,
            'eliminado' => true,
        ]);

 ...
 * Delegado: Validación → DTO → Service → Transformer
 * Reducción: 818 líneas → 280 líneas (-66%)
 * 
 * Refactorización FASE 4.2: 12 de febrero de 2026
 */
class VentaController extends Controller
{
    ...
    #[OA\Post(
        ...
        description: 'Registra una nueva venta con detalles y calcula automáticamente totales',
        security: [['sanctum' => []]],
        tags: ['Ventas']
    )]
    public function store(StoreVentaRequest $request): JsonResponse
    {
        ...
        try {
            // Convertir request validado en DTO
            $dto = VentaCreateDTO::fromRequest($request);
            
            // Delegartoda la lógica al servicio
    ...
    public function update(UpdateVentaRequest $request, int $id): JsonResponse
    {
        ...
        try {
            ...
            return response()->json(
                VentaResource::make($venta->fresh())->resolve()
            );
        } catch (\Throwable $e) {
            return response()->json([

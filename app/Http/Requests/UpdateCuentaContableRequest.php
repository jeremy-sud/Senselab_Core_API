    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'codigo' => [
                ...
                'string',
                'max:50',
                Rule::unique('cuentas_contables', 'codigo')
                    ->where(function ($query) use ($empresaId) {
                        return $query->where('empresa_id', $empresaId)->where('eliminado', 0);
                    })
                    ->ignore($cuentaId)
            ],
            'tipo_cuenta_id' => ['sometimes', 'integer', 'exists:tipos_cuentas,id'],
            'cuenta_padre_id' => ['nullable', 'integer', 'exists:cuentas_contables,id'],
            'permite_movimientos' => ['sometimes', 'boolean'],
            'saldo_actual' => ['sometimes', 'numeric'],
